...
export function ConfirmProvider({ children }) {
  ...
  const confirm = useCallback(
    ({ title, message, tone = 'danger', confirmText = 'Confirm', cancelText = 'Cancel' }) =>
      new Promise((resolve) => setRequest({ title, message, tone, confirmText, cancelText, resolve })),
    []
  );
        ...
        <div className="modal-overlay" onClick={() => handle(false)}>
          <div className="modal-panel" style={{ maxWidth: 420 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <button className={`btn btn-sm ${request.tone === 'danger' ? 'btn-danger' : 'btn-primary'}`} onClick={() => handle(true)}>
                {request.confirmText}
              </button>
            </div>
          </div>
